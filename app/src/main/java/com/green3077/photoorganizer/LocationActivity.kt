...
import com.green3077.photoorganizer.databinding.ActivityLocationBinding
...
/** "나라별 정리" — 해외에서 GPS와 함께 찍힌 사진을 나라 단위로 모아 보여준다. */
class LocationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = LocationGroupAdapter { group -> openLocationDetail(group) }
    ...
    private fun showContent(hasGroups: Boolean) {
        ...
        if (!hasGroups) {
            binding.emptyTitle.text = getString(R.string.empty_location_title)
            binding.emptySubtitle.text = getString(R.string.empty_location_subtitle)
        }
    }
