...
- **날짜별 정리(날짜별 탭)**: 갤러리 전체를 스캔해서 "몇 개 연도에 걸쳐 같은 월/일에 찍힌
  사진들"만 추려 다가오는 날짜순으로 보여줍니다. 상세 화면을 열면 연도별로 묶여서 나오고,
  같은 월/일이 여러 연도에 걸쳐 있으면 위쪽에 연도 필터 칩이 나타나 한 연도만 골라 볼 수
  있습니다.
